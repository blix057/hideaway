#!/usr/bin/env python3
"""
Hideaway - Remote iPhone App Blocker (Fixed Bundled Version)
...
Fixed version with simplified, compatible payload structures.
"""
...
class SupervisedProfileGenerator:
    """Profile generator for MDM-supervised devices with simplified payloads"""
    ...
    def create_app_blocking_profile(self, blocked_apps, profile_name="Focus Mode"):
        """Create a simplified configuration profile that blocks specific apps"""
        
        profile_uuid = str(uuid.uuid4())
        restrictions_uuid = str(uuid.uuid4())
        
        # Create a simplified profile structure that's more compatible
        profile = {
            "PayloadContent": [],
            "PayloadDescription": f"Blocks selected apps to improve focus and productivity",
            "PayloadDisplayName": profile_name,
            "PayloadIdentifier": f"com.hideaway.{profile_name.lower().replace(' ', '').replace('(', '').replace(')', '')}",
            "PayloadOrganization": "Hideaway",
            "PayloadRemovalDisallowed": False,  # Allow removal to avoid issues
            "PayloadType": "Configuration",
            "PayloadUUID": profile_uuid,
            "PayloadVersion": 1
        }
        
        # Simplified restrictions payload - just focus on app blocking
        restrictions_payload = {
            ...
            # Simple blacklist - this is the most compatible approach
            "blacklistedAppBundleIDs": blocked_apps
        }
        
        profile["PayloadContent"].append(restrictions_payload)
        
        # Only add web filtering if we have a few specific apps to avoid complexity
        if any(app_id in blocked_apps for app_id in ["com.burbn.instagram", "com.google.ios.youtube", "com.zhiliaoapp.musically"]):
            # Add simplified web content filter
            web_filter_uuid = str(uuid.uuid4())
            blocked_domains = []
            
            # Only add domains for apps we know work well
            if "com.burbn.instagram" in blocked_apps:
                blocked_domains.extend(["instagram.com", "www.instagram.com"])
            if "com.google.ios.youtube" in blocked_apps:
                blocked_domains.extend(["youtube.com", "www.youtube.com", "youtu.be"])
            if "com.zhiliaoapp.musically" in blocked_apps:
                blocked_domains.extend(["tiktok.com", "www.tiktok.com"])
            
            if blocked_domains:
                web_filter_payload = {
                    "PayloadDisplayName": "Website Restrictions",
                    "PayloadDescription": "Blocks websites related to restricted apps",
                    ...
                    "FilterType": "BuiltIn",
                    "AutoFilterEnabled": False,  # Disable auto-filtering to avoid issues
                    "FilterBrowsers": True,
                    "FilterSockets": False,  # Disable socket filtering to reduce complexity
                    
                    # Simple blocked URLs list
                    "DenyListURLs": blocked_domains
                }
...
class iPhoneSetup:
    """iPhone setup and enrollment automation with simplified profiles"""
    ...
    def create_enrollment_profile(self, device_name="iPhone"):
        """Create simplified iPhone enrollment profile and save to Desktop"""
        print("📱 Creating enrollment profile...")
        
        profile_uuid = str(uuid.uuid4())
        
        # Simplified enrollment profile - just basic structure
        # This version focuses on being a simple configuration profile
        profile = {
            "PayloadContent": [],
            "PayloadDescription": "Hideaway iPhone Enrollment Profile - Install this to prepare your iPhone for app blocking",
        ...
        # Add a simple certificate trust payload (this helps with MDM trust)
        cert_uuid = str(uuid.uuid4())
        cert_payload = {
            "PayloadDisplayName": "Hideaway Certificate",
            "PayloadDescription": "Certificate for Hideaway MDM system",
            "PayloadIdentifier": "com.hideaway.cert",
            "PayloadType": "com.apple.security.root",
            "PayloadUUID": cert_uuid,
            "PayloadVersion": 1,
            # Note: In a real implementation, you'd include the actual certificate data here
            # For now, this creates a basic structure that iOS can handle
        }
        
        profile["PayloadContent"].append(cert_payload)
        ...
        print("4. This prepares your iPhone for app blocking profiles")
...
class HideawayController:
    """Main Hideaway controller application with fixed profiles"""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Hideaway - iPhone App Blocker (Fixed)")
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="🎯 Hideaway (Fixed)", font=("Arial", 16, "bold"))
        ...
        self.log("Hideaway (Fixed) started. Ready to control your iPhone!")
        self.log("🔧 This version uses simplified, compatible payload structures")
    ...
    def test_connection(self):
        """Test connection to device (simplified)"""
        ...
        if not device_id:
            ...
            messagebox.showinfo("Demo Mode", "No device ID provided. Running in demo mode - profiles will be saved to Desktop.")
            return
        
        # Try to connect (this would normally contact nanomdm server)
        try:
    ...
    def generate_blocking_profile(self, block_apps=True):
        """Generate simplified iOS configuration profile to block/unblock apps"""
        ...
        if block_apps:
            ...
            profile = self.profile_generator.create_app_blocking_profile(
                selected_bundles, 
                f"Hideaway Block {', '.join(app_names)}"[:50]  # Keep name short
            )
        else:
    ...
    def send_profile_to_device(self, profile_content):
        """Save profile to Desktop (simplified approach)"""
        ...
        self.log("📱 Transfer this profile to your iPhone and install it")
...
class HideawayLauncher:
    """Main launcher interface for Hideaway (Fixed Version)"""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Hideaway - Setup & Launch (Fixed)")
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="🎯 Hideaway (Fixed)", font=("Arial", 20, "bold"))
        title_label.grid(row=0, column=0, columnspan=2, pady=(0, 20))
...
• Fixed payload content issues"""
    ...
    def generate_profiles(self):
        ...
        try:
            ...
            # Pre-defined focus modes (simplified)
            focus_modes = {
                "Social Media Detox": [
    ...
    def show_help(self):
        ...
        help_text = """🎯 Hideaway (Fixed) - Setup Guide

🔧 What's Fixed:
• Simplified payload structures for better iOS compatibility
• Removed complex payloads that cause errors
• Focused on core app blocking functionality
• All profiles saved to Desktop

📱 STEP 1: iPhone Setup
1. Click "Setup iPhone" button
2. A simple enrollment profile will be saved to your Desktop
3. Transfer it to your iPhone and install

🎛️ STEP 2: Use Controller  
1. Click "Launch Hideaway" button
2. Select apps to block
3. Profiles will be saved to Desktop
4. Transfer to iPhone and install

💡 Key Improvements:
• Fixed payload content errors
• Better compatibility across iOS versions
• Simplified profile structures
• Manual profile installation (more reliable)

📁 All profiles are saved to your Desktop for easy access!"""
        
        help_window = tk.Toplevel(self.root)
        help_window.title("Hideaway (Fixed) - Help")
...
def main():
    print("🎯 Hideaway (Fixed) - Starting...")
    print("🔧 This version uses simplified payload structures for better compatibility")
